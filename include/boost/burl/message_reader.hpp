...
private:
    static capy::io_task<>
    refill_impl(S& stream, parser& pr);

    static capy::io_task<>
    read_header_impl(S& stream, parser& pr);

    static capy::io_task<std::string_view>
    read_body_impl(S& stream, parser& pr);
...
refill_impl(S& stream, parser& pr)
{
    ...
    // Always report the octets first: a read may deliver data alongside
    // eof or a failure, and dropping them would desync the framing.
    pr.commit(n);
    if(ec == capy::cond::eof)
    {
        pr.commit_eof();
...
read_body_impl(S& stream, parser& pr)
{
    // Test first: read_header_impl is idempotent, but reaching it costs a
    // frame, and the header is already parsed on all but the first call.
    if(!pr.got_header())
    ...
    MB buffers)
{
    ...
    for(;;)
    {
        ...
        {
            // buffer_slice rejects rvalue sequences, so name the window.
            auto const mbs = bp.data();
            auto [rec, rn] = co_await stream.read_some(
                capy::buffer_slice(mbs, 0, lim));

            // Always report the octets: they are in the caller's memory
            // whether the read succeeded or not, and the parser has to
            // account for them.
            pr.commit_direct(rn);
    ...
    MB buffers)
{
    ...
    while(total < total_size)
    {
        ...
        dest.consume(n);
        total += n;
        // A contingency which still completed the transfer is a success:
        // report it only when the sequence was not filled.
